<?php
    $pageTitle = "Category Products List";
    $cssFile = "css/displayProductsCategory.css";
...
<?php
    ...
    echo '<div class="breadcrumb">
        ...
        <a href="displayProductsCategory.php?category='. urlencode($_GET['category']) .'">'.$category.'</a>
    </div>';


    //dispaly products category wisely
    echo '<div class="display-products-category-container">';
    
    $sql = "SELECT p.product_id, p.name, p.category, p.price, p.image_path, COALESCE(SUM(s.quantity),0) AS quantity FROM products p 
        LEFT JOIN stocks s ON p.product_id = s.product_id WHERE p.category = ? GROUP BY p.product_id, p.name, p.category, p.price, p.image_path;";
    ...
    $productResults = mysqli_stmt_get_result($stmt);
        ...
        if (mysqli_num_rows($productResults)>0) {
            ...
            while ($row=mysqli_fetch_assoc($productResults)) {
                ...
                echo '<a href="viewProductPurchase.php?product_id=' . urlencode($row['product_id']) . '&category=' . urlencode($row['category']) .  '&name=' . urlencode($row['name']) . '" class="product-link">';
