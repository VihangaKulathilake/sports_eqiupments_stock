<?php
    ...
    $cssFile="css/displayProducts.css";
...
<?php
    ...
    echo '<div class="display-products-container">';
        ...
        $sql="SELECT p.product_id, p.name, p.category, p.price, p.image_path, COALESCE(SUM(s.quantity),0) AS quantity
            FROM products p LEFT JOIN stocks s ON p.product_id = s.product_id GROUP BY p.product_id, p.name, p.category, p.price, p.image_path;";
